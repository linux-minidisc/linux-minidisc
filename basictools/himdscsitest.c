	...
	else
	{
	    ...
	    else
	    {
	    	...
	    	if(!wait_for_unit_ready(scgp))
	           	...
	           	printf("himd successfully formated");
		}
	}
}
	...
	else
	{
	    ...
	    else
	    {
	    	...
	    	if(!wait_for_unit_ready(scgp))
	           	...
	           	printf("medium successfully erased");
		}
	}
}
...
void get_time(SCSI * scgp, char * command, char * errstr)
{
    ...
    {
		read_time(tm, buffer);
		fprintf(stderr, "device time is: %s", asctime(tm));
	}
}
...
int main(int argc, char ** argv)
{
    ...
    if(argc < 3)
    {
        printf("Please specify command, following commands are supported:\n\n");
    ...
    else if(strcmp(argv[2],"capacity") == 0)
    {
        ...
        if(capacity == 0)
            fputs("Error: cannot read capacity", stderr);
        else
            fprintf(stderr, "Capacity : %d MB", capacity);
    }
